...
- **参数模型**: [`Interaction`](../models/interaction.md) — `__slots__` 包含: `id`, `application_id`, `type`, `scene`, `chat_type`, `event_id`, `data` (嵌套 `_Data`: `type`, `resolved` → `_Resolved` 包含 `button_id`, `button_data`, `message_id`, `user_id`, `feature_id`), `guild_id`, `channel_id`, `user_openid`, `group_openid`, `group_member_openid`, `timestamp`, `version`
