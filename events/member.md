...
- **参数模型**: [`Member`](../models/user.md) — `__slots__` 包含: `user` (嵌套 `_User`: `id`, `username`, `avatar`, `bot`, `union_openid`, `union_user_account`), `nick`, `roles`, `joined_at`, `event_id`, `guild_id`
...
- **参数模型**: [`Member`](../models/user.md) — `__slots__` 同 `on_guild_member_add`
