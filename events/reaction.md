...
- **参数模型**: [`Reaction`](../models/other.md#reaction-表情表态) — `__slots__` 包含: `user_id`, `channel_id`, `guild_id`, `emoji` (嵌套 `_Emoji`: `id`, `type`), `target` (嵌套 `_Target`: `id`, `type`), `event_id`
...
- **参数模型**: [`Reaction`](../models/other.md#reaction-表情表态) — `__slots__` 同 `on_message_reaction_add`
