...
- **参数模型**: [`GroupMessage`](../models/message.md#groupmessage-领域模型) — `__slots__`（继承自 `BaseMessage` + 自有）: `content`, `id`, `message_reference` (嵌套 `_MessageRef`: `message_id`), `mentions` (列表 `_User`), `attachments` (列表 `_Attachments`), `msg_seq`, `timestamp`, `event_id`, `author` (嵌套 `_User`: `member_openid`), `group_openid`
...
- **参数模型**: [`GroupManageEvent`](../models/other.md#groupmanageevent-领域模型) — `__slots__` 包含: `event_id`, `timestamp`, `group_openid`, `op_member_openid`
...
- **参数模型**: [`GroupManageEvent`](../models/other.md#groupmanageevent-领域模型) — `__slots__` 同 `on_group_add_robot`
...
- **参数模型**: [`C2CMessage`](../models/message.md#c2cmessage-领域模型) — `__slots__`（继承自 `BaseMessage` + 自有）: `content`, `id`, `message_reference` (嵌套 `_MessageRef`: `message_id`), `mentions` (列表 `_User`), `attachments` (列表 `_Attachments`), `msg_seq`, `timestamp`, `event_id`, `author` (嵌套 `_User`: `user_openid`)
...
- **参数模型**: [`C2CManageEvent`](../models/other.md#c2cmanageevent-领域模型) — `__slots__` 包含: `event_id`, `timestamp`, `openid`
...
- **参数模型**: [`C2CManageEvent`](../models/other.md#c2cmanageevent-领域模型) — `__slots__` 同 `on_friend_add`
...
- **参数模型**: [`PublicAudio`](../models/other.md#publicaudio-领域模型) — `__slots__` 包含: `guild_id`, `channel_id`, `channel_type`, `user_id`
...
- **参数模型**: [`PublicAudio`](../models/other.md#publicaudio-领域模型) — `__slots__` 同 `on_audio_or_live_channel_member_enter`
